...
export default function AdminNotices() {
      ...
      for (const endpoint of endpointsToTry) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(`Endpoint ${backendUrl}${endpoint} returned status ${response.status}`)
          }
        } catch (endpointError) {
  ...
  const runDiagnostics = async () => {
    try {
      ...
      try {
        ...
        toast.error(error instanceof Error ? `Backend connection error: ${error.message}` : 'Failed to connect to backend')
      }
    } catch (error) {
      ...
      for (const endpoint of possibleEndpoints) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(`Upload endpoint ${endpoint} returned status ${response.status}`)
          }
        } catch (endpointError) {
          console.error(`Error with upload endpoint ${endpoint}:`, endpointError)
        }
      }
  ...
  const handleSaveNotice = async () => {
    ...
    try {
      ...
      const possibleEndpoints = isEditing && currentNotice 
        ? [
      ...
      // If we reach here, all endpoints failed
      throw new Error(`Failed to ${isEditing ? 'update' : 'create'} notice. All endpoints failed.`)
    } catch (error) {
      ...
      for (const endpoint of possibleEndpoints) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(`Delete endpoint ${endpoint} returned status ${response.status}`)
          }
        } catch (endpointError) {
          console.error(`Error with delete endpoint ${endpoint}:`, endpointError)
        }
      }
      ...
      <div className="grid gap-4">
        {notices.length === 0 && !isLoading && (
          <div className="p-4 text-center">
            <p className="text-muted-foreground">No notices found. Create your first notice by clicking "Add Notice".</p>
          </div>
        )}
